...
use App\Models\Kategori; // Import model Kategori (jika ada kategori)
...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.index', compact('totalUsers', 'totalKak', 'totalKategoriProgram', 'totalPending', 'totalDisetujui', 'totalDitolak'));
    }


    public function users(Request $request)
    {
        $users = User::with('kategori');
        $kategoris = Kategori::all(); // Ambil semua data kategori untuk dropdown
        ...
        return view('admin.users', compact('users', 'kategoris', 'fromDate', 'toDate'));
    }

    public function kategori()
    {
        // Ambil data kategori dan hitung jumlah user terkait
        $kategoris = \App\Models\Kategori::withCount('users')->get();
    ...
    {
        $kaks = \App\Models\Draft::where('status', '!=', 'draft')->with('kategori'); // Ambil data KAK
        // Ambil nilai dari form
        $fromDate = $request->input('fromDate');
        $toDate = $request->input('toDate');
        // Tambahkan filter tanggal jika tersedia
        ...
        return view('admin.daftar', compact('kaks', 'fromDate', 'toDate'));  // Tampilkan halaman daftar.blade.php
    }
    ...
    public function laporan(Request $request)
    {
        ...
        return view('admin.laporan', compact('kaks', 'fromDate', 'toDate', 'totalPending', 'totalDisetujui', 'totalDitolak'));
    }

    public function faq()
    {
        return view('admin.faq');  // Menampilkan halaman faq.blade.php
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'kategori_id' => 'required|exists:kategori_program,id',
        ]);
        ...
        try {
            ...
            return redirect()->back()->with('error', 'Gagal mengubah user: ' . $e->getMessage());
        }
    }
    ...
    public function storeKategori(Request $request)
    {
        $request->validate([
            ...
            'status' => 'required|in:plt,definitif',
        ]);
        ...
        try {
            ...
            return redirect()->route('admin.kategori')->with('error', 'Gagal menambahkan kategori: ' . $e->getMessage());
        }
    }
    ...
    public function updateKategori(Request $request, $id)
    {
        $request->validate([
            'nama_divisi' => 'required|string|max:255',
            'status' => 'required|in:plt,definitif', // Validasi status
        ]);
        ...
        try {
            ...
            return redirect()->route('admin.kategori')->with('error', 'Gagal mengubah kategori: ' . $e->getMessage());
        }
    }
